...
- (void)handleSystemMessage:(QBChatMessage *)message {
    ...
    if (message.messageType == QMMessageTypeCreateGroupDialog) {
        ...
        [self messagesWithChatDialogID:message.dialogID completion:^(QBResponse *response, NSArray *messages) {
            ...
            [strongSelf.dialogsMemoryStorage addChatDialog:dialogToAdd andJoin:self.isAutoJoinEnabled completion:^(QBChatDialog *addedDialog, NSError *error) {
                //
...
- (void)createGroupChatDialogWithName:(NSString *)name photo:(NSString *)photo occupants:(NSArray *)occupants
    ...
    NSMutableSet *occupantIDs = [NSMutableSet setWithArray:occupants];
...
//    }
    ...
    NSLog(@"%@",name.accessibilityElements );
    
    
    NSLog(@"%@",name.accessibilityHint );
    
      NSLog(@"%@",name.accessibilityValue );
    
    if (name.accessibilityValue.length>0) {
        chatDialog.data=@{@"class_name": @"customparameter",@"category": name.accessibilityValue};
        
    }
    else{
        chatDialog.data=@{@"class_name": @"customparameter", @"category": @"notset"};
         ...
         completion:(QBChatCompletionBlock)completion
{
    ...
    NSLog(@"%@",message.accessibilityHint);
    
    
    if (message.accessibilityHint.length>0) {
        NSMutableDictionary *custDic = [[NSMutableDictionary alloc]init];
        [custDic setValue:message.accessibilityHint forKey:@"isHidden"];
        
        
        message.customParameters=custDic;
