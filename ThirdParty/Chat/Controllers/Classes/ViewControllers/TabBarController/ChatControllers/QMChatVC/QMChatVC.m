...
- (void)viewDidLoad {
  ...
  //  NSUInteger oponentID = [[QMApi instance] occupantIDForPrivateChatDialog:self.dialog];
   // self.opponentUser = [[QMApi instance] userWithID:oponentID];
    ...
    self.lblheader.text=  [NSString stringWithFormat:@"%@\n%@" ,self.dialog.name ,[[self.dialog valueForKey:@"data"] valueForKey:@"category"]];
    ;
    self.lblheader.numberOfLines=2;
    self.lblTOList.userInteractionEnabled=YES;
    UITapGestureRecognizer *tapGestureRecognizer = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(labelTapped:)];
    tapGestureRecognizer.numberOfTapsRequired = 1;
    [self.lblTOList addGestureRecognizer:tapGestureRecognizer];
  
   
    //self.lblTOList.text =opponent.fullName;
    ...
    if (self.dialog.type == QBChatDialogTypePrivate) {
        ...
        self.title = [NSString stringWithFormat:@"%@(%@)" ,self.dialog.name ,[[self.dialog valueForKey:@"data"] valueForKey:@"category"]];
        
        
        // self.onlineTitle.statusLabel.text = [[self.dialog valueForKey:@"data"] valueForKey:@"category"];
        self.onlineTitle.hidden=YES;
    }
...
- (void) labelTapped: (UITapGestureRecognizer *)recognizer
{
    ...
    QMGroupDetailsController *groupDetailVC=[objStoryboard  instantiateViewControllerWithIdentifier:@"QMGroupDetailsController"];
                      ...
                      date:(NSDate *)date
{
    ...
    if (self.dialog.lastMessageUserID ==message.senderID && ![self.dialog.lastMessageText isEqualToString:@"Notification message"]  ) {
        
        if (![self.chatSectionManager chatSectionAtIndex:0]) {
            hiddenString = @"N";
        }
        else{
                hiddenString = @"Y";
    ...
    message.accessibilityHint = hiddenString;
...
- (NSAttributedString *)attributedStringForItem:(QBChatMessage *)messageItem {
    ...
    // comment on 17/6/16 change the color of text msg
    UIColor *textColor = [messageItem senderID] == self.senderID ? [UIColor whiteColor] : [UIColor colorWithRed:0.0f/255.0f green:111.0f/255.0f blue:255.0f/255.0f alpha:1.0f];
...
- (QMChatCellLayoutModel)collectionView:(QMChatCollectionView *)collectionView layoutModelAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (class == [QMChatOutgoingCell class] ||
        ...
        //        if (!DELE.senderID) {
        //            DELE.isFirst=YES;
        //            DELE.senderID = [NSString stringWithFormat:@"%lu", (unsigned long)item.senderID];
        //
        //        }
        //
        //
        //        if (DELE.isFirst==NO) {
        //            if ([DELE.senderID isEqualToString:[NSString stringWithFormat:@"%lu", (unsigned long)item.senderID]]) {
...
- (void)collectionView:(QMChatCollectionView *)collectionView configureCell:(UICollectionViewCell *)cell forIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([cell isKindOfClass:[QMChatOutgoingCell class]] || [cell isKindOfClass:[QMChatAttachmentOutgoingCell class]]) {
        ...
        if ([[message.customParameters valueForKey:@"isHidden"]isEqualToString:@"N"]) {
            placeholder = [UIImage imageNamed:@"placeholder_single"];
        }
        ...
        [(QMChatCell *)cell avatarView].image = placeholder;
        
       
            [QBRequest downloadFileWithID:sender.blobID successBlock:^(QBResponse *response, NSData *fileData) {
                ...
                if (fileData != nil){
                    ...
                    [(QMChatCell *)cell avatarView].image = image;
                    [(QMChatCell *)cell avatarView].layer.cornerRadius = [(QMChatCell *)cell avatarView].frame.size.height/2;
                    [(QMChatCell *)cell avatarView].layer.masksToBounds=YES;
